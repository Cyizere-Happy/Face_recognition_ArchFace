...
Software-only (no MQTT / servo). For the turret scanner use src/recognize.py.

Run:
    python -m src.recognize_live [--threshold 0.65]
...
def main() -> None:
    ...
    try:
        ...
        cap = None
        for cam_idx in [get_cam_index(), 1, 0, 2]:
        ...
        if cap is None or not cap.isOpened():
            raise RuntimeError("Camera not available. Checked FALCON_CAM_INDEX, 1, 0, 2.")
